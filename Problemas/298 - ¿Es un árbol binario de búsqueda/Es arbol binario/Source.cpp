...
template <typename T>
sol<T> es_abbr(bintree<T> const& arbol) {
	sol<T> res;
	if (arbol.empty()) {
		res.abb = true;
	}
	else {
		sol<T> iz = es_abbr(arbol.left());
		sol<T> dr = es_abbr(arbol.right());

		res.min = arbol.left().empty() ? arbol.root() : iz.min;
		res.max = arbol.right().empty() ? arbol.root() : dr.max;
...
template <typename T>
bool es_abb(bintree<T> const& arbol) {
	return es_abbr(arbol).abb;
}
