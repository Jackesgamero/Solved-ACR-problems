...
struct Musico {
	...
	int instrumento;
	int compartido;
	Musico() { partitura = 0; instrumento = 0; compartido = 0; }
	Musico(int _i, int _c){ partitura = 1; instrumento = _i; compartido = _c; }
	bool operator<(const Musico &other) const{
		return compartido > other.compartido;}
};
...
bool solve() {
	...
	PriorityQueue<Musico> musicos;
	int inst;

	for (size_t i = 0; i < n; ++i) {
		std::cin >> inst;
		Musico m = Musico(inst, inst);
		musicos.push(m);
	}


	for (size_t i = 0; i < p - n; ++i) {
		Musico m = musicos.top(); musicos.pop();

		++m.partitura;
		if ((m.instrumento % m.partitura) == 0) m.compartido = m.instrumento / m.partitura;
		else m.compartido = (m.instrumento / m.partitura) + 1;

		musicos.push(m);
	}

	std::cout << musicos.top().compartido << "\n";
	return true;
}
